...
namespace KenobiMp3Parser
{
        public static class Mp3MetadataParser
        {
            ...
            private static Mp3Frame ReadMp3Frame(Stream stream)
            {
                ...
                if (vBits == 0b01)
                    throw new InvalidHeaderException(header.ToArray(), stream.Position);
                if (lBits == 0b00)
                    throw new InvalidHeaderException(header.ToArray(), stream.Position);
        ...
        public static Mp3FileInfo ParseMP3File(Stream stream)
            {
                ...
                int b;
                int failedFrames = 0;
                int failedFramesConsecutive = 0;
                bool previousFailed = false;
                int framesRead = 0;
                Mp3Frame prevFrame = default;
                        ...
                        if ((next & 0xE0) == 0xE0)
                        {
                            ...
                            catch (ArgumentException)
                            {
                                ...
                                if (previousFailed)
                                    failedFramesConsecutive++;
                                previousFailed = true;
                                if (failedFrames == MaxFailedFrames || failedFramesConsecutive == MaxConsecutiveFailedFrames)
                                {
